...
function createFlow(manager, scopeReporter, stepConfiguration, stepDefinition) {
	if (stepDefinition.steps) {
		for (const stepName in stepDefinition.steps) {
			const subStepDefinition = stepDefinition.steps[stepName];

			const stepType = stepDefinition.type;
			if (!stepType) {
				throw new Error(`The step '${stepName}' has no 'type' defined`);
			}

			// creates a new step instance for this flow
			const StepClass = manager.steps[stepType];
			if (!StepClass) {
				throw `The step-type '${stepType}' in the step '${stepName}' could not be find. There is no step registered under this name.`;
			}

			return new StepClass(manager, scopeReporter, stepName, subStepDefinition);
		}
	}
}
