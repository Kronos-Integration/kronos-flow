...
const Flow = {
	 ...
	 */
	endpointFor(expression, problems) {
		let res = expression.match(/^(.+):(.+)/); // service:endpoint
		...
		if (res) {
			...
			res = expression.match(/^(.+)\/(.+)/);
			if (res) { // step/endpoint
				const step = this.steps.services[res[1]];
				if (step) {
			...
			if (subStepDefinition.endpoints) {
				for (const endpointName in subStepDefinition.endpoints) {
					...
					if (target && typeof target === 'string') {
						if (res = target.match(/^(.+):(.+)/)) { // connect against service:endpoint
							const problems = [];
							const targetEndpoint = this.endpointFor(target, problems);

							if (targetEndpoint) {
								endpoint.connected = targetEndpoint;
							} else {
								const mandatory = endpointConfig === 'string' ? true : endpointConfig.mandatory === undefined ? true : false;
								if (mandatory) {
						...
						if (res = target.match(/^(.+)\/(.+)/)) { // optional step:
							...
							if (step.type === 'kronos-flow') {
								// If the step itself is a flow and the target aims to a step from the flow, than nothing is to do
							}

							if (!targetStep && targetStepName === name) {
								// Nothing to do. The step references the flow itself. This will be handled next
							} else if (!targetStep && step.type === 'kronos-flow') {
								// In this case we need to test if the target aims to a step of the flow
								if (step.steps.get(targetStepName)) {
								...
								if (targetEndpoint) {
									if (endpoint.isOut) {
										...
										throw new Error(`Target endpoint '${targetEndpointName}' does not fit with '${endpoint}'`);
									}
								} else {
							...
							throw new Error(`Endpoint target '${endpoint.target}' of endpoint '${endpointName}' is not of right format`);
						}
	...
	/**
	 * Assign a logger endpoint to each of the steps in the flow.
	 * It will only assign the default logger if the log endPoint is not
	 * already connected.
	 */
	_assignLogger() {
		// Check if there is a logger services
		if (this.manager.services.logger) {
			const logEndpoint = this.manager.services.logger.log;
