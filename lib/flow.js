...
module.exports = {
	...
	_initialize(manager, scopeReporter, name, def, endpoints, props) {
		...
		for (const stepName in def.steps) {
			steps.set(stepImpl.createStep(manager, scopeReporter, def.steps[stepName], stepName));
		}
		...
		for (const step of steps.values()) {
			...
			for (const en in step.endpoints) {
				scopeReporter.enterScope('endpoint', en);
					...
					if (res = e.target.match(/^([^\/]+)\/(.*)/)) {
						...
						if (targetStep) {
							...
							if (targetEndpoint) {
								...
								scopeReporter.error('Target endpoint not found', 'step', sid, res[2]);
							}
						} else {
							scopeReporter.error('Target step not found', 'step', sid, res[1]);
						}
