				...
				for (const endpointName in subStepDefinition.endpoints) {
					...
					if (target && typeof target === 'string') {
						if (res = target.match(/^(.+):(.+)/)) { // connect against service:endpoint
							...
							if (!service) {
								if (!endpointConfig.mandatory) {
									// TODO remove optional unconnected endpoint from step ?
								} else {
									throw new Error(`Service '${res[1]}' not found`);
								}
							} else {
								const targetEndpoint = service.endpoints[res[2]];
								if (!targetEndpoint) {
									throw new Error(`Endpoint '${res[2]}' of service ${res[1]} not found`);
								}
