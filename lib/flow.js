...
const Flow = {
	...
	_start() {
		return Promise.all(this.outstandingConnections).then(ocs => {
			...
			return Promise.all(normalSteps.map(s => s.start())).then(() =>
				Promise.all(inboundSteps.map(s => s.start()))
			);
		});
	},
	...
	_stop() {
		const [normalSteps, inboundSteps] = stepsByType(this.steps);
		return Promise.all(inboundSteps.map(s => s.stop())).then(() =>
			Promise.all(normalSteps.map(s => s.stop()))
		);
	},
	...
	toJSONWithOptions(options) {
		...
		json.steps = {};
		this.steps.values().forEach(subStep => json.steps[subStep.name] = subStep.toJSONWithOptions(options));
