...
const Flow = {
	...
	initialize(manager, scopeReporter, name, stepDefinition, endpoints, props) {
		...
		for (const subStepName in stepDefinition.steps) {
			...
			scopeReporter.enterScope('step', subStepName);
			try {
				steps.set(subStepName, manager.getStepInstance(subStepDefinition));
			} catch (e) {
				scopeReporter.exception(e);
			}
		...
		for (const step of steps.values()) {
			...
			for (const endpointName in step.endpoints) {
				...
				let res;
				if (endpoint.target && typeof endpoint.target === 'string') {
		...
		// now assign a logger to each log endpoint
		this._assignLogger(manager, steps);
	},
	 ...
	 */
	updateEndpoints() {
		// do nothing. The flow must not overwrite the original step of an endpoint
	},
	 ...
	 */
	_assignLogger(manager, steps) {
		// get the config from the service manager
		let loggerStep;
		loggerStep = manager.serviceGet(KEY_REGISTER_SERVICE_LOGGER);

		if (!loggerStep) {
			try {
				loggerStep = manager.getStepInstance(DEFAULT_LOGGER_DEFINITION);
			} catch (err) {
				// do nothing. If no step is registered it is OK
			}
			if (loggerStep) {
				// store the step as the default logger
				manager.serviceRegister(KEY_REGISTER_SERVICE_LOGGER, loggerStep);
			}
		}
		...
		if (loggerStep) {
			...
			for (const step of steps.values()) {
				if (!step.endpoints.log.isConnected()) {
					logEndpoint.connect(step.endpoints.log);
				}
			}
		}
	...
	_connectEndpoints() {
		...
		const doConnect = function (step) {
			for (const endPointName in step.endpoints) {
				const endpoint = step.endpoints[endPointName];
				if (endpoint.target && typeof endpoint.target === 'object') {
					endpoint.connect(endpoint.target);
				}
			}
		};
		...
		mapForEachValue(this.steps, step => doConnect(step));
	},
