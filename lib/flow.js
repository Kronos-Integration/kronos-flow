...
const Flow = {
	 ...
	 */
	initialize(manager, scopeReporter, name, stepDefinition, props) {
	 ...
	 */
	createEndpoints(scopeReporter, stepDefinition) {
		...
		this._connectEndpoints(scopeReporter, stepDefinition);
		this._assignLogger(scopeReporter, stepDefinition);
	},

	_connectEndpoints(scopeReporter, stepDefinition) {
					...
					if (target && typeof target === 'string') {
						...
						if (res = target.match(/^(.+)\/(.+)/)) { // optional step:
							...
							if (!targetStep && targetStepName === name) {
								...
								if (targetEndpoint) {
									endpoint.connect(targetEndpoint);
								} else {
